...
final class InMemoryLogsCache<T: Codable> {
...
private static func fileURL() throws -> URL {
  ...
  init(isDebug: Bool) {
    self.isDebug = isDebug
    do {
      let data = try Data(contentsOf: InMemoryLogsCache.fileURL())
      try FileManager.default.removeItem(at: InMemoryLogsCache.fileURL())
